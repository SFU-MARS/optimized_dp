...
class ROV_WaveBwds_6D(DynamicalSystem):
    def __init__(self, x=[0, 0, 0, 0, 0, 0], uMin=np.array([-6, -15]), uMax=np.array([6, 15]),
                 pMin=np.array([-0.2, -0.15]), pMax=np.array([0.2, 0.15]),
    ...
    def opt_ctrl(self, t, spat_deriv):
        ...
        with hcl.if_(self.uMode == "min"):
            ...
            parSum1[0] = (1 / (self.m - self.X_udot)) * spat_deriv[2] * self.B[0, 0] + (1 / (self.m - self.Z_wdot)) * \
                         spat_deriv[3] * self.B[1, 0]
            with hcl.if_(parSum1[0] > 0):
                uOpt1[0] = self.uMin[0]
            with hcl.elif_(parSum1[0] < 0):
                uOpt1[0] = self.uMax[0]

            parSum2[0] = (1 / (self.m - self.X_udot)) * spat_deriv[2] * self.B[0, 1] + (1 / (self.m - self.Z_wdot)) * \
                         spat_deriv[3] * self.B[1, 1]
    ...
    def dynamics(self, t, state, uOpt, dOpt):  # Assume order of state is (x_a, z_a, u_r, w_r, x, z)
        ...
        x3_dot[0] = (1 / (self.m - self.X_udot)) * (
                    -Phi_11[0] * (self.b - self.X_udot) * state[2] + (self.b - self.m) * (
                        G1[0] * self.omega * hcl.sin(sigma[0])) + \
                    Phi_11[0] * (state[2] + G1 * hcl.cos(sigma[0])) + Phi_21[0] * (
                                state[3] + G1[0] * (-hcl.sin(sigma[0]))) - (self.X_u + self.X_uu * my_abs(state[2])) *
                    state[2] + \
        ...
        x4_dot[0] = (1 / (self.m - self.Z_wdot)) * (-Phi_22 * (self.b - self.Z_wdot) * state[3] + \
                                                    ...
                                                    (-(self.W - self.Buoy)) - (
                                                                self.Z_w + self.Z_ww * my_abs(state[3])) * state[3] + \
    ...
    def Hamiltonian(self, t_deriv, spatial_deriv):
        return t_deriv[0] * spatial_deriv[0] + t_deriv[1] * spatial_deriv[1] + t_deriv[2] * spatial_deriv[2] + t_deriv[
            3] * spatial_deriv[3] \
