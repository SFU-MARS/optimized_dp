...
""" 7D 1 vs. 3 AttackerDefender DYNAMICS IMPLEMENTATION 
 xA11_dot = vA * u11
 xA12_dot = vA * u12
 xD11_dot = vD * d11
 xD12_dot = vD * d12
...
class AttackerDefender1v3_7D:
    def __init__(self, x=[0, 0, 0, 0, 0, 0, 0], uMin=-1, uMax=1, dMin=-1,
                 dMax=1, uMode="min", dMode="max", speed_a=1.0, speed_d=1.5):
        """Creates 1 Attacker and 3 Defenders with 1 defender could only move in 1 direction
           The controls are the control inputs of the Attackers.
           The disturbances are the control inputs of the Defender.
        Args:
            x (list, optional): Initial state . Defaults to [0, 0, 0, 0, 0, 0, 0].
    ...
    def dynamics(self, t, state, uOpt, dOpt):
        ...
        xD3_dot[0] = self.speed_d * dOpt[4]

        return xA11_dot[0], xA12_dot[0], xD11_dot[0], xD12_dot[0], xD21_dot[0], xD22_dot[0], xD3_dot[0]
    ...
    def opt_ctrl(self, t, state, spat_deriv):
        ...
        return opt_a1[0], opt_a2[0], in3[0], in4[0], in5[0], in6[0], in7[0]
    ...
    def opt_dstb(self, t, state, spat_deriv):
        ...
        dstb_len3 = hcl.sqrt(deriv5[0] * deriv5[0])
    ...
    def optDstb_inPython(self, spat_deriv):
        ...
        dstb_len3 = np.sqrt(deriv7*deriv7)
        ...
        return (opt_d1, opt_d2, opt_d3, opt_d4, opt_d5)

    def capture_set1(self, grid, capture_radius, mode):
        # The capture relationship between the A and the D1
    ...
    def capture_set2(self, grid, capture_radius, mode):
        # The capture relationship between the A and the D2
    ...
    def capture_set3(self, grid, capture_radius, mode):
        # The capture relationship between the A and the D3
        #TODO: since the D3 could only move in one direction, the y position of D3 should be fixed as a constant
        data = np.power(grid.vs[0] - grid.vs[6], 2) + np.power(grid.vs[1] - 0, 2)
        if mode == "capture":
            return np.sqrt(data) - capture_radius
        if mode == "escape":
            return capture_radius - np.sqrt(data)
