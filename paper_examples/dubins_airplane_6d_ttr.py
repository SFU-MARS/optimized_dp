...
from odp.dynamics import DubinsAirplane6D
...
# Dubins Airplane 6D TTR example 
X_RANGE = [-8.0, +8.0]
Y_RANGE = [-8.0, +8.0]
Z_RANGE = [0.0, +8.0]
PSI_RANGE = [-np.pi, +np.pi]  # heading angle psi
GAMMA_RANGE = [-np.pi/2., +np.pi/2.]  # flight path angle gamma
PHI_RANGE = [-np.pi/2., +np.pi/2.]  # roll angle phi
PTS_EACH_DIM = np.array([30, 30, 30, 30, 30, 30])
...
with open(log_file_name, "w") as f:
    ...
    f.write(f"Z_RANGE: {Z_RANGE}\n")
    f.write(f"PSI_RANGE: {PSI_RANGE}\n")
    f.write(f"GAMMA_RANGE: {GAMMA_RANGE}\n")
    f.write(f"PHI_RANGE: {PHI_RANGE}\n")
...
## dynamics
dubinairplane6d = DubinsAirplane6D(x=[0, 0, 0, 0, 0, 0],
                                   uMin=[-1, -1],
                                   uMax=[1, 1],
                                   dMin=[0, 0, 0],
                                   dMax=[0, 0, 0],
...
## grids
grid = Grid(minBounds=np.array([X_RANGE[0], Y_RANGE[0], Z_RANGE[0], PSI_RANGE[0], GAMMA_RANGE[0], PHI_RANGE[0]]), 
            maxBounds=np.array([X_RANGE[1], Y_RANGE[1], Z_RANGE[1], PSI_RANGE[1], GAMMA_RANGE[1], PHI_RANGE[1]]),
            dims=6,
...
## target set
target = CylinderShape(grid=grid,
                       ignore_dims=[2,3,4,5],
...
## solve
ttr_result = TTRSolver(dynamics_obj=dubinairplane6d,
                       grid=grid,
                       multiple_value=target,
                       epsilon=0.001,
