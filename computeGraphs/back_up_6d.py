...
def spa_derivX(i, j, k, V, g):
    left_deriv1 = hcl.scalar(0, "left_deriv1")
    right_deriv1 = hcl.scalar(0, "right_deriv1")
    with hcl.if_(i == 0):
        left_boundary1 = hcl.scalar(0, "left_boundary1")
        left_boundary1[0] = V[k, j, i] + my_abs(V[k, j, i + 1] - V[k, j, i]) * my_sign(
            V[k, j, i])
        left_deriv1[0] = (V[k, j, i] - left_boundary1[0]) / g.dx[0]
        right_deriv1[0] = (V[k, j, i + 1] - V[k, j, i]) / g.dx[0]
    with hcl.elif_(i == V.shape[2] - 1):
        right_boundary1 = hcl.scalar(0, "right_boundary1")
        right_boundary1[0] = V[k, j, i] + my_abs(V[k, j, i] - V[k, j, i - 1]) * my_sign(
            V[k, j, i])
        left_deriv1[0] = (V[k, j, i] - V[k, j, i - 1]) / g.dx[0]
        right_deriv1[0] = (right_boundary1[0] - V[k, j, i]) / g.dx[0]
    with hcl.elif_(i != 0 and i != V.shape[2] - 1):
        left_deriv1[0] = (V[k, j, i] - V[k, j, i-1]) / g.dx[0]
        right_deriv1[0] = (V[k, j, i+1] - V[k, j, i]) / g.dx[0]
    return left_deriv1[0], right_deriv1[0]


def spa_derivY(i, j, k, V, g):
    left_deriv2 = hcl.scalar(0, "left_deriv2")
    right_deriv2 = hcl.scalar(0, "right_deriv2")
    with hcl.if_(j == 0):
        left_boundary2 = hcl.scalar(0, "left_boundary2")
        left_boundary2[0] = V[k, j, i] + my_abs(V[k, j + 1, i] - V[k, j, i]) * my_sign(
            V[k, j, i])
        left_deriv2[0] = (V[k, j, i] - left_boundary2[0]) / g.dx[1]
        right_deriv2[0] = (V[k, j + 1, i] - V[k, j, i]) / g.dx[1]
    with hcl.elif_(j == V.shape[1] - 1):
        right_boundary2 = hcl.scalar(0, "right_boundary2")
        right_boundary2[0] = V[k, j, i] + my_abs(V[k, j, i] - V[k, j - 1, i]) * my_sign(
            V[k, j, i])
        left_deriv2[0] = (V[k, j, i] - V[k, j - 1, i]) / g.dx[1]
        right_deriv2[0] = (right_boundary2[0] - V[k, j, i]) / g.dx[1]
    with hcl.elif_(j != 0 and j != V.shape[1] - 1):
        left_deriv2[0] = (V[k, j, i] - V[k, j - 1, i]) / g.dx[1]
        right_deriv2[0] = (V[k, j + 1, i] - V[k, j, i]) / g.dx[1]
    return left_deriv2[0], right_deriv2[0]

def spa_derivT(i, j, k, V, g):
    left_deriv3 = hcl.scalar(0, "left_deriv3")
    right_deriv3 = hcl.scalar(0, "right_deriv3")
    with hcl.if_(k == 0):
        left_boundary3 = hcl.scalar(0, "left_boundary3")
        # left_boundary[0] = V_init[i,j,50]
        left_boundary3[0] = V[V.shape[0] - 1, j, i]
        left_deriv3[0] = (V[k, j, i] - left_boundary3[0]) / g.dx[2]
        right_deriv3[0] = (V[k + 1, j, i] - V[k, j, i]) / g.dx[2]
    with hcl.elif_(k == V.shape[0] - 1):
        right_boundary3 = hcl.scalar(0, "right_boundary3")
        right_boundary3[0] = V[0, j, i]
        left_deriv3[0] = (V[k, j, i] - V[k-1, j, i]) / g.dx[2]
        right_deriv3[0] = (right_boundary3[0] - V[k, j, i]) / g.dx[2]
    with hcl.elif_(k != 0 and k != V.shape[0] - 1):
        left_deriv3[0] = (V[k, j, i] - V[k-1, j, i]) / g.dx[2]
        right_deriv3[0] = (V[k+1, j, i] - V[k, j, i]) / g.dx[2]
    return left_deriv3[0], right_deriv3[0]
...
########################## 4D Graph definition #################################
def graph_4D(V_new, V_init, deriv_diff1, deriv_diff2, deriv_diff3, deriv_diff4, x1, x2, x3, x4, t):
    # Maximum derivative for each dim
    max_deriv1 = hcl.scalar(-1e9, "max_deriv1")
    max_deriv2 = hcl.scalar(-1e9, "max_deriv2")
    max_deriv3 = hcl.scalar(-1e9, "max_deriv3")
            ...
            with hcl.for_(0, V_init.shape[1], name="j") as j:
                with hcl.for_(0, V_init.shape[2], name="k") as k:
                    with hcl.for_(0, V_init.shape[3], name="l") as l:
                        ...
                        # Calculate alpha
                        alpha1 = my_max(my_abs(dx_LL1[0]), my_abs(dx_LU1[0]))
                        alpha1 = my_max(alpha1[0], my_abs(dx_UL1[0]))
                        alpha1 = my_max(alpha1[0], my_abs(dx_UU1[0]))
                        
                        alpha2 = my_max(my_abs(dx_LL2[0]), my_abs(dx_LU2[0]))
                        alpha2 = my_max(alpha2[0], my_abs(dx_UL2[0]))
                        alpha2 = my_max(alpha2[0], my_abs(dx_UU2[0]))
                        
                        alpha3 = my_max(my_abs(dx_LL3[0]), my_abs(dx_LU3[0]))
                        alpha3 = my_max(alpha3[0], my_abs(dx_UL3[0]))
                        alpha3 = my_max(alpha3[0], my_abs(dx_UU3[0]))
                        
                        alpha4 = my_max(my_abs(dx_LL4[0]), my_abs(dx_LU4[0]))
                        alpha4 = my_max(alpha4[0], my_abs(dx_UL4[0]))
                        alpha4 = my_max(alpha4[0], my_abs(dx_UU4[0]))
