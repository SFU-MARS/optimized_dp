...
# Update the value function at position (i,j,k,l,m)
# iVals:      holds index values (i,j,k,l,m) that correspond to state values (si,sj,sk,sl,sm)
...
def updateVopt(i, j, k, l, m, iVals, sVals, actions, Vopt, intermeds, trans, interpV, gamma, bounds, goal, ptsEachDim, useNN):
    p = hcl.scalar(0, "p")

    with hcl.for_(0, actions.shape[0], name="a") as a:
        # set iVals equal to (i,j,k,l,m) and sVals equal to the corresponding state values (si,sj,sk,sl,sm)
        updateStateVals(i, j, k, l, m, iVals, sVals, bounds, ptsEachDim)
        # call the transition function to obtain the outcome(s) of action a from state (si,sj,sk,sl,sm)
        ...
        with hcl.for_(0, trans.shape[0], name="si") as si:
            ...
            with hcl.if_(useNN[0] == 1):
                # convert the state values of the successor state (si,sj,sk,sl,sm) into indeces (ia,ja,ka,la,ma)
                stateToIndex(sVals, iVals, bounds, ptsEachDim)
                # if (ia,ja,ka,la,ma) is within the state space, add its discounted value to the Q value
                with hcl.if_(hcl.and_(iVals[0] < Vopt.shape[0], iVals[1] < Vopt.shape[1], iVals[2] < Vopt.shape[2])):
                    with hcl.if_(hcl.and_(iVals[3] < Vopt.shape[3], iVals[4] < Vopt.shape[4])):
                        with hcl.if_(hcl.and_(iVals[0] >= 0, iVals[1] >= 0, iVals[2] >= 0, iVals[3] >= 0, iVals[4] >= 0)):
                            intermeds[a] += (gamma[0] * (p[0] * Vopt[iVals[0], iVals[1], iVals[2], iVals[3], iVals[4]]))

        # maximize over each Q value to obtain the optimal value
        Vopt[i,j,k,l,m] = -1000000
        with hcl.for_(0, intermeds.shape[0], name="r") as r:
            with hcl.if_(Vopt[i,j,k,l,m] < intermeds[r]):
                Vopt[i,j,k,l,m] = intermeds[r]
...
# Sets iVals equal to (i,j,k,l,m) and sVals equal to the corresponding state values
def updateStateVals(i, j, k, l, m, iVals, sVals, bounds, ptsEachDim):
    iVals[0] = i
    iVals[1] = j
    iVals[2] = k
    iVals[3] = l
...
def value_iteration_5D():
                            ...
                            with hcl.for_(0, Vopt.shape[2], name="k") as k:
                                with hcl.for_(0, Vopt.shape[3], name="l") as l:
                                    with hcl.for_(0, Vopt.shape[4], name="m") as m:
                                        ...
                                        updateVopt(i, j, k, l, m, iVals, sVals, actions, Vopt, intermeds, trans, interpV, gamma, bounds, goal, ptsEachDim, useNN)
                                ...
                                with hcl.for_(1, Vopt.shape[2] + 1, name="k") as k:
                                    with hcl.for_(0, Vopt.shape[3], name="l") as l:
                                        with hcl.for_(0, Vopt.shape[4], name="m") as m:
                                            ...
                                            updateVopt(i2, j2, k2, l, m, iVals, sVals, actions, Vopt, intermeds, trans, interpV, gamma, bounds, goal, ptsEachDim, useNN)
                                ...
                                with hcl.for_(0, Vopt.shape[2], name="k") as k:
                                    with hcl.for_(0, Vopt.shape[3], name="l") as l:
                                        with hcl.for_(0, Vopt.shape[4], name="m") as m:
                                            ...
                                            updateVopt(i2, j, k, l, m, iVals, sVals, actions, Vopt, intermeds, trans, interpV, gamma, bounds, goal, ptsEachDim, useNN)
                                ...
                                with hcl.for_(0, Vopt.shape[2], name="k") as k:
                                    with hcl.for_(0, Vopt.shape[3], name="l") as l:
                                        with hcl.for_(0, Vopt.shape[4], name="m") as m:
                                            ...
                                            updateVopt(i, j2, k, l, m, iVals, sVals, actions, Vopt, intermeds, trans, interpV, gamma, bounds, goal, ptsEachDim, useNN)
                                ...
                                with hcl.for_(1, Vopt.shape[2] + 1, name="k") as k:
                                    with hcl.for_(0, Vopt.shape[3], name="l") as l:
                                        with hcl.for_(0, Vopt.shape[4], name="m") as m:
                                            ...
                                            updateVopt(i, j, k2, l, m, iVals, sVals, actions, Vopt, intermeds, trans, interpV, gamma, bounds, goal, ptsEachDim, useNN)
                                ...
                                with hcl.for_(0, Vopt.shape[2], name="k") as k:
                                    with hcl.for_(0, Vopt.shape[3], name="l") as l:
                                        with hcl.for_(0, Vopt.shape[4], name="m") as m:
                                            ...
                                            updateVopt(i2, j2, k, l, m, iVals, sVals, actions, Vopt, intermeds, trans, interpV, gamma, bounds, goal, ptsEachDim, useNN)
                                ...
                                with hcl.for_(1, Vopt.shape[2] + 1, name="k") as k:
                                    with hcl.for_(0, Vopt.shape[3], name="l") as l:
                                        with hcl.for_(0, Vopt.shape[4], name="m") as m:
                                            ...
                                            updateVopt(i2, j, k2, l, m, iVals, sVals, actions, Vopt, intermeds, trans, interpV, gamma, bounds, goal, ptsEachDim, useNN)
                                ...
                                with hcl.for_(1, Vopt.shape[2] + 1, name="k") as k:
                                    with hcl.for_(0, Vopt.shape[3], name="l") as l:
                                        with hcl.for_(0, Vopt.shape[4], name="m") as m:
                                            ...
                                            updateVopt(i, j2, k2, l, m, iVals, sVals, actions, Vopt, intermeds, trans, interpV, gamma, bounds, goal, ptsEachDim, useNN)
    ...
    ptsEachDim = hcl.placeholder(tuple([5]), name="ptsEachDim", dtype=hcl.Float())
    sVals      = hcl.placeholder(tuple([5]), name="sVals", dtype=hcl.Float())
    iVals      = hcl.placeholder(tuple([5]), name="iVals", dtype=hcl.Float())
    ...
    sVals      = hcl.asarray(np.zeros([5]))
    iVals      = hcl.asarray(np.zeros([5]))
    ...
    file_name = "hcl_value_iteration_5D_" + str(int(c[0])) + "_iterations_by" + ("_Interpolation" if UD._useNN[0] == 0 else "_NN")
