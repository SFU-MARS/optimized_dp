...
def updateVopt(obj, current_indices, actions, V, gamma, bounds, ptsEachDim):
    ...
    with hcl.for_(0, actions.shape[0], name="a") as a:
        ...
        with hcl.for_(0, transition_matrix.shape[0], name="si") as si:
            ...
            intermeds[a] += (gamma[0] * (p[0] * V[iVals[0], iVals[1], iVals[2], iVals[3], iVals[4]]))
...
def continuousStateToDiscreteIndices(sVals, bounds, ptsEachDim):
    iVals = hcl.compute((5,), lambda *x: 0, "iVals") 
...
def value_iteration_5D(MDP_object, pts_each_dim, bounds, 
                                ...
                                with hcl.for_(0, Vopt.shape[3], name="l") as l:
                                    with hcl.for_(0, Vopt.shape[4], name="m") as m:
                                        ...
                                        if sweep_id & 16:
                                            m = Vopt.shape[4] - m - 1
                                        indices = (i, j, k, l, m)
                                        Vopt[i, j, k, l, m] = updateVopt(MDP_object, indices, actions, Vopt, 
    ...
    ptsEachDim = hcl.placeholder((5,), name="ptsEachDim", dtype=hcl.Float())
