...
def updatePhi(i, j, k, l, my_object, phi, constraint, g, x1, x2, x3, x4):
    ...
    dOpt = my_object.opt_dstb(0, (x1[i], x2[j], x3[k], x4[l]),
                              (dV_dx1[0], dV_dx2[0], dV_dx3[0], dV_dx4[0]))
    ...
    phi[i, j, k, l] = my_max(phi[i, j, k, l], constraint[i, j, k, l])
        ...
        with hcl.for_(0, phi.shape[1], name="j") as j:
            with hcl.for_(0, phi.shape[2], name="k") as k:
                with hcl.for_(0, phi.shape[3], name="l") as l:
                    ...
                    phi[0, j, k, l] = my_max(phi[0, j, k, l], constraint[0, j, k, l])
                    ...
                    phi[phi.shape[0] - 1, j, k, l] = my_max(phi[phi.shape[0] - 1, j, k, l], constraint[phi.shape[0] - 1, j, k, l])

    if 1 not in g.pDim:
        with hcl.for_(0, phi.shape[0], name="i3") as i3:
            with hcl.for_(0, phi.shape[2], name="k") as k:
                with hcl.for_(0, phi.shape[3], name="l") as l:
                    tmp1 = hcl.scalar(0, "tmp1")
                    tmp1[0] = 2 * phi[i3, 1, k, l] - phi[i3, 2, k, l]
                    tmp1[0] = my_max(tmp1[0], phi[i3, 2, k, l ])
                    phi[i3, 0, k, l] = my_min(tmp1[0], phi[i3, 0, k, l])
                    phi[i3, 0, k, l] = my_max(phi[i3, 0, k, l], constraint[i3, 0, k, l])

                    tmp2 = hcl.scalar(0, "tmp2")
                    tmp2[0] = 2 * phi[i3, phi.shape[1] - 2, k, l] - phi[i3, phi.shape[1] - 3, k, l]
                    tmp2[0] = my_max(tmp2[0], phi[i3, phi.shape[1] - 3, k, l])
                    phi[i3, phi.shape[1] - 1, k, l] = my_min(tmp2[0], phi[i3, phi.shape[1] - 1, k, l])
                    phi[i3, phi.shape[1] - 1, k, l] = my_max(phi[i3, phi.shape[1] - 1, k, l], constraint[i3, phi.shape[1] - 1, k, l])
    ...
    if 2 not in g.pDim:
        with hcl.for_(0, phi.shape[0], name="i3") as i3:
            with hcl.for_(0, phi.shape[1], name="j") as j:
                with hcl.for_(0, phi.shape[3], name="l") as l:
                    tmp1 = hcl.scalar(0, "tmp1")
                    tmp1[0] = 2 * phi[i3, j, 1, l] - phi[i3, j, 2, l]
                    tmp1[0] = my_max(tmp1[0], phi[i3, j, 2, l])
                    phi[i3, j, 0, l] = my_min(tmp1[0], phi[i3, j, 0, l])
                    phi[i3, j, 0, l] = my_max(phi[i3, j, 0, l], constraint[i3, j, 0, l])

                    tmp2 = hcl.scalar(0, "tmp2")
                    tmp2[0] = 2 * phi[i3, j, phi.shape[2] - 2, l] - phi[i3, j, phi.shape[2] - 3, l]
                    tmp2[0] = my_max(tmp2[0], phi[i3, j, phi.shape[2] - 3, l])
                    phi[i3, j, phi.shape[2] - 1, l] = my_min(tmp2[0], phi[i3, j, phi.shape[2] - 1, l])
                    phi[i3, j, phi.shape[2] - 1, l] = my_max(phi[i3, j, phi.shape[2] - 1, l], constraint[i3, j, phi.shape[2] - 1, l])
    ...
    if 3 not in g.pDim:
        with hcl.for_(0, phi.shape[0], name="i3") as i3:
            with hcl.for_(0, phi.shape[1], name="j") as j:
                with hcl.for_(0, phi.shape[2], name="k") as k:
                    tmp1 = hcl.scalar(0, "tmp1")
                    tmp1[0] = 2 * phi[i3, j, k, 1] - phi[i3, j, k, 2]
                    tmp1[0] = my_max(tmp1[0], phi[i3, j, k, 2])
                    phi[i3, j, k, 0] = my_min(tmp1[0], phi[i3, j, k, 0])
...
def TTR_4D(my_object, g):
    def solve_phiNew(phi, constraint, x1, x2, x3, x4):
                    ...
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            with hcl.for_(l_l, h_l, name="l") as l:
                                updatePhi(i, j, k, l, my_object, phi, constraint, g, x1, x2, x3, x4)
                    ...
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            with hcl.for_(l_l, h_l, name="l") as l:
                                ...
                                j2 = phi.shape[1] - j - 1
                                k2 = phi.shape[2] - k - 1
                                updatePhi(i2, j2, k2, l, my_object, phi, constraint, g, x1, x2, x3, x4)
                EvalBoundary(phi, constraint, g)
                    ...
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            with hcl.for_(l_l, h_l, name="l") as l:
                                j2 = phi.shape[1] - j - 1
                                k2 = phi.shape[2] - k - 1
                                updatePhi(i, j2, k2, l, my_object, phi, constraint, g, x1, x2, x3, x4)
                EvalBoundary(phi, constraint, g)
                    ...
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            with hcl.for_(l_l, h_l, name="l") as l:
                                i2 = phi.shape[0] - i - 1
                                j2 = phi.shape[1] - j - 1
                                updatePhi(i2, j2, k, l, my_object, phi, constraint, g, x1, x2, x3, x4)
                EvalBoundary(phi, constraint, g)
                    ...
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            with hcl.for_(l_l, h_l, name="l") as l:
                                i2 = phi.shape[0] - i - 1
                                l2 = phi.shape[3] - l - 1
                                updatePhi(i2, j, k, l2, my_object, phi, constraint, g, x1, x2, x3, x4)
                EvalBoundary(phi, constraint, g)
                    ...
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            with hcl.for_(l_l, h_l, name="l") as l:
                                j2 = phi.shape[1] - j - 1
                                l2 = phi.shape[3] - l - 1
                                updatePhi(i, j2, k, l2, my_object, phi, constraint, g, x1, x2, x3, x4)
                EvalBoundary(phi, constraint, g)
                    ...
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            with hcl.for_(l_l, h_l, name="l") as l:
                                k2 = phi.shape[2] - k - 1
                                l2 = phi.shape[3] - l - 1
                                updatePhi(i, j, k2, l2, my_object, phi, constraint, g, x1, x2, x3, x4)
                EvalBoundary(phi, constraint, g)
                    ...
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            with hcl.for_(l_l, h_l, name="l") as l:
                                ...
                                k2 = phi.shape[2] - k - 1
                                l2 = phi.shape[3] - l - 1
                                updatePhi(i2, j2, k2, l2, my_object, phi, constraint, g, x1, x2, x3, x4)
                EvalBoundary(phi, constraint, g)
    ...
    s = hcl.create_schedule([phi, constraint, x1, x2, x3, x4], solve_phiNew)
